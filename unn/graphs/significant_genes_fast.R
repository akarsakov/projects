...
for (i in 1:nrow(cancerData))
{
  ...
  threshold = mean(adj_mat)
  adj_mat = adj_mat > threshold
  g = graph.adjacency(adj_mat)
  cancerGraphMetrics[i,] = c("cancer", diameter(g), centralization.evcent(g)$centralization)
  # vertex degree
  degrees = degree(g)
...
for (i in 1:nrow(testHealthData))
{
  ...
  adj_mat = adj_mat > threshold
  g = graph.adjacency(adj_mat)
  healthGraphMetrics[i,] = c("healthy", diameter(g), centralization.evcent(g)$centralization)
  # vertex degree
  degrees = degree(g)
...
cancerVertex = cancerVertex / (2*nrow(cancerData))
healthVertex = healthVertex / (2*nrow(testHealthData))

print("print first significant genes")
print("for cancer:")
res = cbind(name, cancerVertex)
names(res) <- c("name", "average degree")
res = res[order(-res[,2]),]
write.csv(res, file="cancer_genes.csv")

print("for helthy:")
res = cbind(name, healthVertex)
names(res) <- c("name", "average degree")
res = res[order(-res[,2]),]
write.csv(res, file="health_genes.csv")